...
def evaluate():
    ...
    # TARGET RATIO GROUPED BY EDUCATION LEVEL
    df = df[[edu, target]]
    rates = df.groupby(by=edu).mean() * 100
    #print(rates)
    rates["TARGET"].plot(x="Type of Education", y="Insolvency rate (%)", kind="bar")
    plt.title("Insolvency rate grouped by Education Level")
    plt.tight_layout(pad = .8)
    plt.show()



    # Beneath code that will be eliminated
